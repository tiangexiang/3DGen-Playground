"""
Standalone inference script for JiT 3DGS diffusion models.

Loads a checkpoint, generates samples, renders them via gsplat, and saves PNGs.
Exposes all sampling knobs available during training validation.

Notes on defaults (matched to how the training code actually noises data):

* ``heun`` / ``euler`` integrate the JiT flow-matching velocity ODE and are
  the consistent samplers for this repo: ``jit/train_gsplat.py`` trains with
  ``x_t = t · x_0 + (1 − t) · ε`` (t ∈ (0, 1), t=0 → noise, t=1 → clean) and
  predicts x₀ (see ``GaussianDiffusion.flow_matching_training_losses``).
  ``ddim`` / ``dpm`` / ``ddpm`` assume the DDPM
  ``x_t = √ᾱ_t · x_0 + √(1-ᾱ_t) · ε`` schedule and will produce garbage on
  checkpoints trained after the flow-matching switch — use them only for
  legacy DDPM checkpoints.

* ``--use_ema`` defaults to ``False``.  For the current JiT-XL/8 checkpoint the
  EMA weights are collapsed toward the mean (std ≈ 0.3–0.45 vs base ≈ 0.9;
  RMSE against clean data is ~30% worse than base).  Prefer base weights.
  Pass ``--use_ema`` explicitly if you want to re-test the EMA.

Usage:
    python jit/infer_gsplat.py \\
        ...
        [--class_label 5]   # omit for random class each sample
"""
...
def main(args: argparse.Namespace) -> None:
    ...
    # the EMA is collapsed toward the mean.  Pass --use_ema to override.
    if args.use_ema and "ema" in ckpt:
        model.load_state_dict(ckpt["ema"])
        logger.warning(
            "Loaded EMA weights (opted in via --use_ema). For the current JiT-XL/8 "
            "checkpoint these predict x0 with ~30%% worse RMSE than base; expect "
    ...
    # Guard: jit/train_gsplat.py now trains with JiT flow-matching noising
    # (x_t = t*x_0 + (1-t)*eps, t=0→noise, t=1→clean) and x0 prediction; only
    # heun/euler integrate the matching velocity ODE. DDPM-family samplers
    # (ddim/dpm/ddpm) assume x_t = sqrt(ab_t)*x_0 + sqrt(1-ab_t)*eps on the
    # OPPOSITE discrete-t convention and will produce degenerate samples on
    # checkpoints trained under flow matching.
    if args.sampler in {"ddim", "dpm", "ddpm"}:
        logger.warning(
            "Sampler '%s' assumes DDPM noising (x_t = sqrt(ab_t)*x_0 + "
            "sqrt(1-ab_t)*eps), but jit/train_gsplat.py trains with JiT "
            "flow-matching (x_t = t*x_0 + (1-t)*eps). Samples will be "
            "degenerate on FM checkpoints; use 'heun' or 'euler' instead.",
            args.sampler,
        )
    ...
    # We need num_points; derive from atlas size (128×128)
    num_points = 128 * 128
    ...
    for i in range(args.num_samples):
        ...
        with torch.no_grad():
            ...
            # rendered: (num_cams, C, H, W) or (1, num_cams, C, H, W) — normalise shape
            if rendered.ndim == 5:
                rendered = rendered[0]  # (num_cams, C, H, W)

        # Save each camera view
        tag = f"sample{i:04d}_class{y_label:03d}_step{step}"
        for cam_idx_pos, cam_idx in enumerate(cam_indices):
            img_tensor = rendered[cam_idx_pos]  # (C, H, W)
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="JiT 3DGS inference: load checkpoint, sample, render, save PNGs.",
    ...
    g_model.add_argument("--use_ema", action=argparse.BooleanOptionalAction, default=False,
                         help="Use EMA weights from checkpoint. Default False: for the current "
                              ...
                              "produce visibly better samples.")

    # ── Sampling ─────────────────────────────────────────────────────────────
    g_sample = p.add_argument_group("Sampling")
    g_sample.add_argument("--sampler", type=str, default="heun", choices=SAMPLER_CHOICES,
                          help="Sampling algorithm. 'heun'/'euler' integrate the JiT "
                               "flow-matching velocity ODE and are consistent with "
                               "flow_matching_training_losses. 'ddim'/'dpm'/'ddpm' assume "
                               "DDPM q_sample and only match legacy DDPM-trained checkpoints.")
    ...
    # CFG — only applies to heun/euler
    ...
    # JiT ODE knobs — only applies to heun/euler
    g_sample.add_argument("--t_eps", type=float, default=5e-2,
                          help="Minimum t value to avoid division by zero in velocity computation. "
                               "Only applies to heun/euler samplers.")
    ...
    # DPM-Solver knobs — only applies to dpm sampler
    ...
    # ── Misc ──────────────────────────────────────────────────────────────────
    p.add_argument("--output_dir", type=str, default="output/infer_results",
...
def _merge_yaml_into_parser_defaults(parser: argparse.ArgumentParser, config_path: str) -> None:
    ...
    # Satisfy 'required' for any arg that now has a YAML-provided default
